...
describe("CrowdFund", function () {
    ...
    describe("部署", function () {
        it("应该正确部署 MyToken 和 CrowdFund", async function () {
    ...
    describe("发起众筹 (launch)", function () {
        it("应该能够成功发起众筹", async function () {
            ...
            if (launchEvent) {
                ...
                expect(parsed?.args[1]).to.equal(creatorAddress);
                expect(parsed?.args[2]).to.equal(FUNDING_GOAL);
            }
    ...
    describe("取消众筹 (cancel)", function () {
        ...
        it("非创建者不应该能够取消众筹", async function () {
            await expect(
                crowdFund.connect(supporter1).cancel(1)
            ).to.be.reverted;
        });
        ...
        it("不应该能够取消已开始的众筹", async function () {
            ...
            await expect(
                crowdFund.connect(creator).cancel(2)
    ...
    describe("撤回认捐 (unpledge)", function () {
        ...
        it("应该能够成功撤回认捐", async function () {
            ...
            expect(unpledgeEvent).to.not.be.undefined;
            ...
            expect(supporter1BalanceAfter).to.equal(supporter1BalanceBefore + unpledgeAmount);
        });
